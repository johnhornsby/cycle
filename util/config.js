...
function loadConfig(grunt) {
	...
	var config = null;
	var potentialPaths = [
		grunt.config('wd') + '/cycleconfig.js',
		grunt.config('wd') + '/assets/cycleconfig.js'
	];
	...
	if (!config) {
		grunt.fail.warn("Unable to locate 'cycleconfig.js' file. Please place it in the root or assets folder.");
		return false;
	}
